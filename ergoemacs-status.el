...
(defun ergoemacs-status-save-file ()
  ...
  (with-temp-file ergoemacs-status-file
    ...
    (let ((print-level nil)
          (print-length nil))
      (insert "(setq ergoemacs-status--hidden-minor-modes '")
      (prin1 ergoemacs-status--hidden-minor-modes (current-buffer))
      (goto-char (point-max))
      (insert ")"))))
...
(defcustom ergoemacs-status-elements
  '((:read-only (ergoemacs-status--read-only) 3 nil "Read Only")
    ...
    (:position (ergoemacs-status-position) 2 "Buffer size")
    ...
    (:major (ergoemacs-status-major-mode-item) nil nil "Major mode")
  ...
  (let* (ret
	...
	(unless (and tmp (stringp tmp) (string= (format-mode-line tmp) ""))
	  ...
	  (setq tmp (propertize
		     (mapconcat
		      (lambda (mm)
			(let ((cur (get-text-property 0 'local-map mm)))
			  (if (not cur) mm
			    (setq cur (copy-keymap cur))
			    (define-key cur [mode-line drag-mouse-1] #'eroemacs-status-drag)
			    (setq mm (propertize mm 'local-map cur))
			    mm)))
		      (ergoemacs-status--property-substrings tmp 'local-map)
		      "")
